...
public interface AiAgentConfigMapper {
    ...
    @Select("SELECT id, account_id, agent_key, name, icon, description, gradient, chips, endpoint_url, auth_type, auth_token, request_template, response_type, timeout_seconds, ui_mode, ui_config_json, usage_location, sort_order, created_at, updated_at FROM ai_agent_config WHERE id = #{id}")
    AiAgentConfig selectById(Long id);

    @ResultMap("agentConfigMap")
    @Select("SELECT id, account_id, agent_key, name, icon, description, gradient, chips, endpoint_url, auth_type, auth_token, request_template, response_type, timeout_seconds, ui_mode, ui_config_json, usage_location, sort_order, created_at, updated_at FROM ai_agent_config WHERE account_id = #{accountId} ORDER BY sort_order, id")
    ...
    @ResultMap("agentConfigMap")
    @Select("SELECT id, account_id, agent_key, name, icon, description, gradient, chips, endpoint_url, auth_type, auth_token, request_template, response_type, timeout_seconds, ui_mode, ui_config_json, usage_location, sort_order, created_at, updated_at FROM ai_agent_config WHERE account_id = #{accountId} AND agent_key = #{agentKey} LIMIT 1")
    AiAgentConfig selectByAccountIdAndKey(@Param("accountId") Long accountId, @Param("agentKey") String agentKey);

    @Insert("INSERT INTO ai_agent_config (account_id, agent_key, name, icon, description, gradient, chips, endpoint_url, auth_type, auth_token, request_template, response_type, timeout_seconds, ui_mode, ui_config_json, usage_location, sort_order) VALUES (#{accountId}, #{agentKey}, #{name}, #{icon}, #{description}, #{gradient}, #{chips, typeHandler=com.example.springboot.config.JsonListTypeHandler}, #{endpointUrl}, #{authType}, #{authToken}, #{requestTemplate}, #{responseType}, #{timeoutSeconds}, #{uiMode}, #{uiConfigJson}, #{usageLocation}, #{sortOrder})")
     ...
     */
    @ResultMap("agentConfigMap")
    @Select("SELECT id, account_id, agent_key, name, icon, description, gradient, chips, endpoint_url, auth_type, auth_token, request_template, response_type, timeout_seconds, ui_mode, ui_config_json, usage_location, sort_order, created_at, updated_at FROM ai_agent_config WHERE (account_id = #{accountId} OR account_id = 0 OR account_id IS NULL) AND agent_key = #{agentKey} ORDER BY account_id DESC LIMIT 1")
