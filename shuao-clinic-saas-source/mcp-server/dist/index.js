 ...
 * 发起带认证的 API 请求
 */
async function makeApiRequest(endpoint, method = "GET", data, params) {
    const headers = {
 ...
 * 格式化分页结果为 Markdown
 */
function formatPageResultMarkdown(title, data, itemFormatter) {
...
// ==================== Tool 注册 ====================
// ---------- query_patients ----------
server.registerTool("query_patients", {
    title: "查询患者列表",
...
Returns:
  患者列表分页数据，包含姓名、性别、年龄、手机号、客户来源等字段。`,
    ...
    try {
        const data = await makeApiRequest("patients/search", "GET", undefined, {
        ...
        const text = formatPageResultMarkdown(`患者搜索结果: ${params.keyword || "全部"}`, data?.data, (item) => {
            const parts = [
...
// ---------- query_appointments ----------
server.registerTool("query_appointments", {
    title: "查询预约列表",
...
Returns:
  预约列表分页数据，包含患者姓名、预约时间、医生、状态等。`,
    ...
    try {
        const data = await makeApiRequest("appointments/selectAll", "GET", undefined, {
        ...
        const text = formatPageResultMarkdown(`预约列表${params.status ? ` [状态: ${params.status}]` : ""}`, data?.data, (item) => {
            const parts = [
...
server.registerTool("query_medical_records", {
    title: "查询病历列表",
...
Returns:
  病历列表分页数据，包含主诉、现病史、诊断、治疗计划等。`,
    ...
    try {
        const data = await makeApiRequest("medical-records/selectByPatientId", "GET", undefined, {
        ...
        const text = formatPageResultMarkdown(`患者病历列表 (患者ID: ${params.patient_id})`, data?.data, (item) => {
            const parts = [
...
// ---------- query_finances ----------
server.registerTool("query_finances", {
    title: "查询财务记录",
...
Returns:
  财务记录分页数据，包含收费项目、金额、患者、收费时间等。`,
    ...
    try {
        const data = await makeApiRequest("finances/all", "GET", undefined, {
        ...
        const text = formatPageResultMarkdown(`财务记录${params.start_date || params.end_date
...
// ---------- query_treatments ----------
server.registerTool("query_treatments", {
    title: "查询治疗记录",
...
Returns:
  治疗记录分页数据，包含治疗内容、牙位、医生、费用等。`,
    ...
    try {
        const data = await makeApiRequest("treatments/selectAll", "GET", undefined, {
        ...
        const text = formatPageResultMarkdown(`治疗记录${params.patient_name ? ` [患者: ${params.patient_name}]` : ""}`, data?.data, (item) => {
            const parts = [
...
// ---------- query_lab_orders ----------
server.registerTool("query_lab_orders", {
    title: "查询加工单",
...
Returns:
  加工单分页数据，包含加工厂、加工内容、状态、交付日期等。`,
    ...
    try {
        const data = await makeApiRequest("lab-orders/search", "GET", undefined, {
            keyword: params.keyword,
            page: params.page,
            size: params.size,
        });
        ...
        const text = formatPageResultMarkdown(`加工单列表${params.keyword ? ` [关键词: ${params.keyword}]` : ""}`, data?.data, (item) => {
            const parts = [
...
// ---------- query_materials ----------
server.registerTool("query_materials", {
    title: "查询耗材库存",
...
Returns:
  耗材分页数据，包含名称、规格、库存数量、预警阈值等。`,
    ...
    try {
        const data = await makeApiRequest("materials/search", "GET", undefined, {
            keyword: params.keyword,
            page: params.page,
            size: params.size,
        });
        ...
        const text = formatPageResultMarkdown(`耗材库存${params.keyword ? ` [关键词: ${params.keyword}]` : ""}`, data?.data, (item) => {
            const parts = [
...
// ---------- query_consultations ----------
server.registerTool("query_consultations", {
    title: "查询咨询/回访记录",
...
Returns:
  咨询记录分页数据，包含咨询时间、咨询项目、意向等级、处理结果等。`,
    ...
    try {
        const data = await makeApiRequest("consultations/search", "GET", undefined, {
            keyword: params.keyword,
            page: params.page,
            size: params.size,
        });
        ...
        const text = formatPageResultMarkdown(`咨询记录${params.keyword ? ` [关键词: ${params.keyword}]` : ""}`, data?.data, (item) => {
            const parts = [
                `## ${item.patient_name || item.name || "未知"} - ${item.consultation_time || "-"}`,
...
// ---------- get_patient_360 ----------
server.registerTool("get_patient_360", {
    title: "获取患者360视图",
...
Returns:
  ...
  - wechatBound: 微信绑定状态`,
    ...
    try {
        const data = await makeApiRequest(`patient360/overview/${params.patient_id}`, "GET");
        if (params.response_format === ResponseFormat.JSON) {
            const text = JSON.stringify(data, null, 2);
            const { text: finalText } = truncateIfNeeded(text);
            return {
        ...
        for (const t of (overview.treatments || []).slice(0, 10)) {
            lines.push(`- ${t.treatment_date || "-"}: ${t.treatment_content || "-"} (¥${t.fee ?? "-"})`);
        }
        lines.push("", `## 预约记录 (${(overview.appointments || []).length}条)`);
        for (const a of (overview.appointments || []).slice(0, 10)) {
            lines.push(`- ${a.appointment_time || "-"}: ${a.appointment_purpose || "-"} [${a.status || "-"}]`);
        }
 ...
 */
function authenticateMcpApiKey(req, res, next) {
...
async function runHttpServer() {
    ...
    app.listen(PORT, () => {
        ...
        console.error(`API Key 鉴权: ${MCP_API_KEY ? "已启用" : "未启用（未配置 MCP_API_KEY）"}`);
    });
}
