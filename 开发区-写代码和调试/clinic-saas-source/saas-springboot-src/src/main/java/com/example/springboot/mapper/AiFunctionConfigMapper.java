...
public interface AiFunctionConfigMapper {
    ...
    @Select("SELECT id, function_key AS functionKey, function_name AS functionName, page_path AS pagePath, icon, is_enabled AS isEnabled, model_name AS modelName, prompt_template_id AS promptTemplateId, extra_config AS extraConfig, sort_order AS sortOrder, create_time AS createTime, update_time AS updateTime FROM ai_function_config ORDER BY sort_order")
    List<AiFunctionConfig> selectAll();

    @ResultMap("functionConfigMap")
    @Select("SELECT id, function_key AS functionKey, function_name AS functionName, page_path AS pagePath, icon, is_enabled AS isEnabled, model_name AS modelName, prompt_template_id AS promptTemplateId, extra_config AS extraConfig, sort_order AS sortOrder, create_time AS createTime, update_time AS updateTime FROM ai_function_config WHERE function_key = #{functionKey} LIMIT 1")
