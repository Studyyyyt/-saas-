...
public interface TreatmentMapper {
    ...
    @Select("<script>" +
            ...
            "  <if test='startDate != null and startDate != \"\"'>AND treatment_date &gt;= #{startDate}</if>" +
            "  <if test='endDate != null and endDate != \"\"'>AND treatment_date &lt;= #{endDate}</if>" +
            "  <if test='status != null and status != \"\"'>AND status = #{status}</if>" +
            "</where>" +
            "ORDER BY treatment_date DESC, id DESC" +
            "</script>")
    List<Treatment> searchTreatments(@Param("patientId") Long patientId,
