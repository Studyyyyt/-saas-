...
public interface AiFunctionMappingMapper {
    ...
    @Select("SELECT id, account_id AS accountId, function_code AS functionCode, function_name AS functionName, agent_key AS agentKey, is_visible_on_page AS isVisibleOnPage, is_visible_on_home AS isVisibleOnHome, sort_order AS sortOrder, created_at AS createdAt, updated_at AS updatedAt FROM ai_function_mapping WHERE id = #{id}")
    AiFunctionMapping selectById(Long id);

    @ResultMap("functionMappingMap")
    @Select("SELECT id, account_id AS accountId, function_code AS functionCode, function_name AS functionName, agent_key AS agentKey, is_visible_on_page AS isVisibleOnPage, is_visible_on_home AS isVisibleOnHome, sort_order AS sortOrder, created_at AS createdAt, updated_at AS updatedAt FROM ai_function_mapping WHERE account_id = #{accountId} ORDER BY sort_order, id")
    List<AiFunctionMapping> selectByAccountId(@Param("accountId") Long accountId);

    @ResultMap("functionMappingMap")
    @Select("SELECT id, account_id AS accountId, function_code AS functionCode, function_name AS functionName, agent_key AS agentKey, is_visible_on_page AS isVisibleOnPage, is_visible_on_home AS isVisibleOnHome, sort_order AS sortOrder, created_at AS createdAt, updated_at AS updatedAt FROM ai_function_mapping WHERE (account_id = #{accountId} OR account_id IS NULL) AND function_code = #{functionCode} ORDER BY account_id DESC LIMIT 1")
    AiFunctionMapping selectByAccountAndCode(@Param("accountId") Long accountId, @Param("functionCode") String functionCode);

    @ResultMap("functionMappingMap")
    @Select("SELECT id, account_id AS accountId, function_code AS functionCode, function_name AS functionName, agent_key AS agentKey, is_visible_on_page AS isVisibleOnPage, is_visible_on_home AS isVisibleOnHome, sort_order AS sortOrder, created_at AS createdAt, updated_at AS updatedAt FROM ai_function_mapping WHERE account_id IS NULL ORDER BY sort_order, id")
