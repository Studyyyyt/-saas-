...
public interface AiPromptTemplateMapper {
    ...
    @Select("SELECT id, scene, name, system_prompt AS systemPrompt, temperature, max_tokens AS maxTokens, response_format AS responseFormat, json_schema AS jsonSchema, extra_config AS extraConfig, is_active AS isActive, version, create_time AS createTime, update_time AS updateTime FROM ai_prompt_template WHERE scene = #{scene} LIMIT 1")
    AiPromptTemplate selectByScene(String scene);

    @ResultMap("promptTemplateMap")
    @Select("SELECT id, scene, name, system_prompt AS systemPrompt, temperature, max_tokens AS maxTokens, response_format AS responseFormat, json_schema AS jsonSchema, extra_config AS extraConfig, is_active AS isActive, version, create_time AS createTime, update_time AS updateTime FROM ai_prompt_template WHERE id = #{id}")
