...
public interface AiModelProviderMapper {
    ...
    @Select("SELECT id, provider_name AS providerName, base_url AS baseUrl, api_key AS apiKey, model_name AS modelName, reasoning_effort AS reasoningEffort, max_output_tokens AS maxOutputTokens, enabled, api_type AS apiType, created_at AS createdAt, updated_at AS updatedAt FROM ai_model_provider ORDER BY id LIMIT 1")
    AiModelProvider selectFirst();

    @ResultMap("modelProviderMap")
    @Select("SELECT id, provider_name AS providerName, base_url AS baseUrl, api_key AS apiKey, model_name AS modelName, reasoning_effort AS reasoningEffort, max_output_tokens AS maxOutputTokens, enabled, api_type AS apiType, created_at AS createdAt, updated_at AS updatedAt FROM ai_model_provider WHERE id = #{id}")
