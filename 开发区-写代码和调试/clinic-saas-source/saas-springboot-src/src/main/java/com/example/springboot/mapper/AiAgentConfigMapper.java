...
public interface AiAgentConfigMapper {
    ...
    @Select("SELECT id, account_id AS accountId, agent_key AS agentKey, name, icon, description, gradient, chips, preset_message AS presetMessage, endpoint_url AS endpointUrl, auth_type AS authType, auth_token AS authToken, request_template AS requestTemplate, response_type AS responseType, timeout_seconds AS timeoutSeconds, ui_mode AS uiMode, ui_config_json AS uiConfigJson, usage_location AS usageLocation, is_visible_on_home AS isVisibleOnHome, sort_order AS sortOrder, created_at AS createdAt, updated_at AS updatedAt FROM ai_agent_config WHERE id = #{id}")
    AiAgentConfig selectById(Long id);

    @ResultMap("agentConfigMap")
    @Select("SELECT id, account_id AS accountId, agent_key AS agentKey, name, icon, description, gradient, chips, preset_message AS presetMessage, endpoint_url AS endpointUrl, auth_type AS authType, auth_token AS authToken, request_template AS requestTemplate, response_type AS responseType, timeout_seconds AS timeoutSeconds, ui_mode AS uiMode, ui_config_json AS uiConfigJson, usage_location AS usageLocation, is_visible_on_home AS isVisibleOnHome, sort_order AS sortOrder, created_at AS createdAt, updated_at AS updatedAt FROM ai_agent_config WHERE account_id = #{accountId} ORDER BY sort_order, id")
    ...
    @ResultMap("agentConfigMap")
    @Select("SELECT id, account_id AS accountId, agent_key AS agentKey, name, icon, description, gradient, chips, preset_message AS presetMessage, endpoint_url AS endpointUrl, auth_type AS authType, auth_token AS authToken, request_template AS requestTemplate, response_type AS responseType, timeout_seconds AS timeoutSeconds, ui_mode AS uiMode, ui_config_json AS uiConfigJson, usage_location AS usageLocation, is_visible_on_home AS isVisibleOnHome, sort_order AS sortOrder, created_at AS createdAt, updated_at AS updatedAt FROM ai_agent_config WHERE account_id = #{accountId} AND agent_key = #{agentKey} LIMIT 1")
     ...
     */
    @ResultMap("agentConfigMap")
    @Select("SELECT id, account_id AS accountId, agent_key AS agentKey, name, icon, description, gradient, chips, preset_message AS presetMessage, endpoint_url AS endpointUrl, auth_type AS authType, auth_token AS authToken, request_template AS requestTemplate, response_type AS responseType, timeout_seconds AS timeoutSeconds, ui_mode AS uiMode, ui_config_json AS uiConfigJson, usage_location AS usageLocation, is_visible_on_home AS isVisibleOnHome, sort_order AS sortOrder, created_at AS createdAt, updated_at AS updatedAt FROM ai_agent_config WHERE (account_id = #{accountId} OR account_id = 0 OR account_id IS NULL) AND agent_key = #{agentKey} ORDER BY account_id DESC LIMIT 1")
