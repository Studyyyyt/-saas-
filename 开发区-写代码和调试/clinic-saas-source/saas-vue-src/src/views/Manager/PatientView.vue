    ...
    <section class="patient-main">
      ...
      <div class="toolbar-shell">
        <div class="toolbar-row">
          ...
          <el-select
            v-if="isAdmin || clinicOptions.length > 1"
...
export default {
  ...
  methods: {
    ...
    async loadClinicOptions() {
      const clinics = (this.currentUser && this.currentUser.clinics) || []
      if (Array.isArray(clinics) && clinics.length > 0) {
        this.clinicOptions = clinics
        return
      }
      try {
        ...
        const res = await axios.get('/clinics/mine', { params: { userId } })
        const data = res && res.data ? res.data : {}
        if (data.code === 200 || data.code === '200') {
          this.clinicOptions = Array.isArray(data.data) ? data.data : []
        } else {
          this.clinicOptions = []
        }
      } catch (error) {
    ...
    buildWorkbenchParams(options = {}) {
      ...
      return {
        ...
        clinicId: this.clinicFilter || undefined,
