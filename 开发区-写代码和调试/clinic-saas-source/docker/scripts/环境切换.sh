...
start_dev() {
    ...
    echo "  · 修改 Java 代码后自动热重启（devtools）"
    echo "  · 修改前端代码后需手动构建（npm run build）并重启前端容器"
    echo "  · 不需要构建镜像，启动速度快"
    echo "  · 适合日常编码、调试、功能开发"
    echo ""

    # 检查前端构建产物是否存在
    local dist_dir="${DOCKER_ROOT}/../saas-vue-src/dist"
    if [ ! -d "$dist_dir" ] || [ -z "$(ls -A "$dist_dir" 2>/dev/null)" ]; then
        show_warn "前端构建产物不存在，需要重新构建..."
        if [ -d "${DOCKER_ROOT}/../saas-vue-src" ]; then
            show_info "正在构建前端..."
            cd "${DOCKER_ROOT}/../saas-vue-src"
            if ! npm install; then
                show_err "npm install 失败"
                exit 1
            fi
            ...
            show_err "找不到前端源码目录: ${DOCKER_ROOT}/../saas-vue-src"
            exit 1
        fi
    fi
