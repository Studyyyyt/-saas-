...
ADD COLUMN IF NOT EXISTS `medical_record_id` bigint DEFAULT NULL COMMENT '关联病历ID' AFTER `status`,
ADD KEY IF NOT EXISTS `idx_appointment_medical_record_id` (`medical_record_id`);

ALTER TABLE `treatment`
ADD COLUMN IF NOT EXISTS `finance_id` bigint DEFAULT NULL COMMENT '关联收费单ID' AFTER `medical_record_id`,
ADD KEY IF NOT EXISTS `idx_treatment_finance_id` (`finance_id`);

ALTER TABLE `medical_records`
ADD COLUMN IF NOT EXISTS `treatment_generated` tinyint DEFAULT 0 COMMENT '是否已生成处置记录：0-否 1-是' AFTER `record_status`;

ALTER TABLE `patient_followup`
ADD COLUMN IF NOT EXISTS `source_type` varchar(20) DEFAULT 'manual' COMMENT '来源：manual/ai' AFTER `summary`;
